...
async function dashboardAnatomy(browser) {
  await withPage(browser, async (page) => {
    ...
    // The verdict is the card's sentence and reads first; the three figures are
    // columns from the left, not pinned to the card's edges (600px of nothing
    // between Net and the pair on a wide screen); the caption sits on the bar's line.
    const order = await page.evaluate(() => { const card = document.querySelector("[data-summary]"); const y = (sel) => Math.round(card.querySelector(sel)?.getBoundingClientRect().top ?? -1); const figs = [...card.querySelectorAll(".text-2xl")].map((e) => e.getBoundingClientRect()); const bar = card.querySelector("[role=progressbar]").getBoundingClientRect(); const cap = card.querySelector("[data-bar-caption]")?.getBoundingClientRect(); return { statusAbove: y("[data-status]") >= 0 && y("[data-status]") < y(".text-2xl"), gap: figs.length === 3 ? Math.round(figs[2].left - figs[1].right) : -1, capOnBar: cap ? Math.abs((cap.top + cap.bottom) / 2 - (bar.top + bar.bottom) / 2) <= 2 && cap.left > bar.right : null }; });
    record("dashboard", "the verdict reads first, the figures are columns from the left, and the caption is on the bar's line", order.statusAbove && order.gap >= 0 && order.gap <= 64 && order.capOnBar === true, `verdict above figures=${order.statusAbove}; income→expenses gap ${order.gap}px; caption on bar=${order.capOnBar}`);
