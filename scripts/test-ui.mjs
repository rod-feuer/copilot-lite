...
async function budgetBars(browser) {
  await withPage(browser, async (page) => {
    ...
    for (const route of ["/categories", "/"]) {
      ...
      const tip = await page.waitForFunction(() => [...document.querySelectorAll("body *")].find((e) => e.children.length === 0 && /through the month/.test(e.textContent))?.textContent ?? null, { timeout: 3000 }).then((h) => h.jsonValue()).catch(() => null);
      record("budget bars", `${where} · hovering the pace line says what it is`, !!tip && /Today: \d+% through the month/.test(tip), tip ?? "no tooltip");
