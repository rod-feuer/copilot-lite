...
async function dashboardAnatomy(browser) {
  await withPage(browser, async (page) => {
    ...
    record("dashboard", "the verdict reads first; the figures are equal columns; the budget panel sits to their right, level, its share above its bar", order.statusAbove && order.pitch.length === 2 && order.pitch[0] === order.pitch[1] && order.panelRight && order.panelLevel && order.capAboveBar === true, `verdict above=${order.statusAbove}; column pitch ${order.pitch.join("/")}px; panel right=${order.panelRight}, level=${order.panelLevel}; caption above bar=${order.capAboveBar}`);
