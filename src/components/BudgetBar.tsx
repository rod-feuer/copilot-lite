...
export function BudgetBar({ spent, budget, pace }: { spent: number; budget: number; pace: number | null }) {
  ...
  return (
    <div className="relative" data-budget-bar>
      ...
      {pace != null && pace > 0 && pace < 1 && (
        ...
        // the fill and the track alike.
        <span
          data-pace
          aria-hidden
          className="absolute -top-0.5 h-3 w-0.5 -translate-x-1/2 rounded-full bg-[var(--foreground)] shadow-[0_0_0_1px_var(--card)]"
          style={{ left: pct(pace * budget) }}
        />
      )}
    </div>
  );
